...
class SmartFilter extends CBitrixComponent
{
	...
	public function executeComponent()
	{
		...
		if(!empty($this->arParams['FILTER_URL']))
			$this->urlString = $this->arParams['FILTER_URL'];
	...
	public function getParsedFilterFromUrl()
	{
		...
		$filterParts = array_chunk(explode('/', $this->urlString), 2);
		...
		foreach ($filterParts as $filterItemCode => $filterItemValues)
		{
			...
			// if(!empty($this->arParams['FIELDS_MAP'][$filterItemCode]))
			// 	$filterItemCode = $this->arParams['FIELDS_MAP'][$filterItemCode];
