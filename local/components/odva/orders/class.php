...
class Orders extends CBitrixComponent
{
	public function getOrders($arFilter = [])
	{
		...
		while ($arSales = $dbSales->Fetch())
		{
			...
			$arSales['DELIVERY_PRICE']      = $this->getDeliveryPrice($arSales["DELIVERY_ID"]);
	...
	{
		$db = \Bitrix\Sale\Delivery\Services\Table::getList(['filter' => ['ACTIVE' => 'Y','ID' => $deliveryId]]);
		return $db->fetch()['CONFIG']['MAIN']['PRICE'];
	}
