...
$result = [];
$global_key = 0;
foreach ($arResult['CATEGORIES'] as $key => $categoryes)
{
	foreach ($categoryes['ITEMS'] as $key => $item)
	{
		if($item['MODULE_ID'] == "main")
		{
			$result['ITEMS'][$global_key] = $item;
		}
		if($item['MODULE_ID'] == "iblock")
		{
			if($item['PARAM2'] == 1)
			{
				$result['ITEMS'][$global_key] = GetIBlockElement($item['ITEM_ID']);
			}
			if($item['PARAM2'] == 2)
			{
				$result['ITEMS'][$global_key] = GetIBlockElement($item['ITEM_ID']);
...
$result['URL'] = $arResult['CATEGORIES']['all']['ITEMS'][0]['URL'];
$arResult = $result;
?>